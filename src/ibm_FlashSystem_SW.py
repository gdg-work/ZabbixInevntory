...
class IBMFlashSystem(inv.ClassicArrayClass):
    ...
    def __FillArrayParams__(self):
        ...
        lCommonParams = self.__sFromArray__('lssystem -delim {}'.format(sDelim)).split('\n')
        oLog.debug("__FillArrayParams__: lCommonParams: " + str(lCommonParams))
        for sName, sVal in (l.split(sDelim) for l in lCommonParams if sDelim in l):
    ...
    def __FillEnclosures__(self):
        ...
        sDelim = ","
        lEnclInfo = [l for l in self.__sFromArray__('lsenclosure -delim {}'.format(sDelim)).split('\n')
                     if len(l.strip()) > 0]
    ...
    def __FillNodes__(self):
        ...
        sDelim = ','
        lsNodesInfo = [l for l in self.__sFromArray__('lsnode -delim {}'.format(sDelim)).split('\n')
                       if len(l.strip()) > 0]
        ...
        for sNodeData in lsNodesInfo:
            ...
            lsThisNodeData = [l for l in
                              self.__sFromArray__('lsnode -delim {0} {1}'.format(sDelim, iNum)).split('\n')
                              if len(l.strip()) > 0]
            for sKey, sVal in (l.split(sDelim) for l in lsThisNodeData):
                if sKey == 'port_id':
                    iPortsCount += 1
            ...
            # hardware components
            lsThisNodeData = [l for l in
                              self.__sFromArray__('lsnodehw -delim {0} {1}'.format(sDelim, iNum)).split('\n')
                              if len(l.strip()) > 0]
            for sKey, sVal in (l.split(sDelim) for l in lsThisNodeData):
                if sKey == 'cpu_count':
                    self.iCPUs = int(sVal)
...
if __name__ == '__main__':
    ...
    oFS.__FillArrayParams__()
    print(str(oFS._dGetArrayInfoAsDict(oFS.dQueries.keys())))
    print(str(oFS._ldGetControllersInfoAsDict()))
    print(str(oFS._ldGetShelvesAsDicts()))
